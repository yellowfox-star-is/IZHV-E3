...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        Vector3 rotation = director.eulerAngles;
        rotation.y -= shotgunSpread / 2;
        for (int i = 0; i < shotgunBullets; i++)
        {
            SpawnBullet(position, Quaternion.Euler(rotation));
            rotation.y += shotgunSpread / (shotgunBullets - 1);
        }
    }
